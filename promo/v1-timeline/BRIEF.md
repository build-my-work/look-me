...
Look Me（看看我）官方宣传视频，三个平行版本中的「一天的时间轴」版。用户委托："做 3 个版本，然后给我看"——autonomous 信号，直接交付成品 MP4。产品是知乎刘看山 IP 的桌面护眼陪伴应用：透明置顶小宠物常驻桌面，本地摄像头视觉检测（不上传），提醒眨眼、远眺（20-20-20）、起身。调性：安静、温和、不打扰——"只在该出现的时候出现"。官网叙事签名是贯穿全页的 Timeline 脊柱（09:00 → 10:24 → 14:07 → 18:30 → 现在），本版本复刻这一结构作为视频主线。

## Assets

素材源目录：`/Users/zhihu/code/m_code/ai/look-me/site/assets/`（已复制到 `capture/assets/`）
- kanshan-distance-break.png — 刘看山站姿立绘（512×640 比例），主角形象
- kanshan-clap-sprite.png — 拍手精灵图，8 帧横排（800% 宽），官网 steps() 播放
- kanshan-sit-sprite.png — 坐下精灵图，8 帧横排
- kanshan-spin-sprite.png — 转圈精灵图，11 帧横排（1100% 宽）
- kanshan-yawn-mouth.png — 打哈欠的嘴部贴片
- kanshan-tear.png — 眼泪贴片（泪流 + 泪滴）
