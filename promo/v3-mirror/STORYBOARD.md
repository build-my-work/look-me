...
- scene: 注意力轨道剧场（官网 rail-demo 同款）：看山从右上角沿虚线轨道缓缓降落、泪流挂脸（tear 贴片 + 泪滴反复坠落）；字幕「25 秒不眨眼——」；眨眼瞬间（画面闪一次眼睑），他立刻擦泪、沿轨道回升，字幕「你一眨眼，他立刻擦泪回升」
- voiceover: ""
- duration: 11s
- transition_in: crossfade
- status: animated
...
- focal: 降落中的看山 + 泪流
- asset_candidates: assets/kanshan-distance-break.png — 立绘; assets/kanshan-tear.png — 泪流与泪滴
- roles: kanshan-distance-break = cutout（轨道主体）· kanshan-tear = supporting（泪流 + 泪滴）
...
Adapt：sub-shape B 无光标飞行——相机跟随看山沿轨道的两段旅程（降→升），旅程本身就是叙事。
Scene 1 (0.0–1.6s): 舞台建立：右上角虚线轨道 SVG self-draw；看山挂泪流出现在顶端（`spring-pop-entrance` 平滑）。
Scene 2 (1.6–5.0s): 相机缓慢下移跟随（`multi-phase-camera` drift）看山沿轨道降落；泪滴 finite 两坠（`dynamic-content-sequencing`）；字幕①「25 秒不眨眼，他从屏幕角落缓缓降落」per-word 随降随落。
Scene 3 (5.0–6.8s): 字幕②「掉着眼泪提醒你」word-swap；降落到位定格半拍（情绪最低点）。
Scene 4 (6.8–9.2s): 回应拍：画面轻闪一拍 + 你图形眼睑一闭一开；看山擦泪（tear 淡出）；字幕③前半「你一眨眼——」落。
Scene 5 (9.2–11.0s): 看山沿轨道回升（相机 drift 回上）；字幕③后半「他立刻擦泪回升，然后安静 25 秒」落；settle hold。
...
- scene: 舞台安静收拢：中央一张玻璃卡，两列逐条落定——「他在本机看的」（你的脸在不在画面里 / 眼睛有没有闭上 / 你有没有起身）与「他永远不会」（保存任何一帧画面 / 把画面传出你的电脑 / 偷看你的屏幕内容）；看山立绘守在卡旁
- voiceover: ""
- duration: 10s
...
- focal: 中央玻璃卡（两列隐私对照）
- asset_candidates: assets/kanshan-distance-break.png — 立绘
- roles: kanshan-distance-break = supporting（卡旁守望）
...
Scene 3 (5.6–7.6s): 看山立绘自卡侧小步入守望位（一次小位移，无其它运动）。
Scene 4 (7.6–10.0s): 底行「不上传 · 不保存 · 没有云端」三拍落；still hold——静是 payload。

## Frame 6 — 让他住进你的桌面

- scene: 雾白底：看山立绘落位并拍一次手（sprite 彩蛋），字标「Look Me · 看看我」+ 副题「刘看山住进你的桌面」，三平台按钮依次亮起；最后一拍：看山眼睑眨一下，全片终
- voiceover: ""
- duration: 8s
...
- focal: 「Look Me · 看看我」字标 + 看山 lockup
- asset_candidates: assets/kanshan-distance-break.png — 立绘; assets/kanshan-clap-sprite.png — 拍手彩蛋
- roles: kanshan-distance-break = cutout · clap-sprite = 彩蛋叠层

narrativeRole: 情感向收尾——不是「买产品」，是「带他回家」；最后一眨眼呼应 Frame 1 的镜像。
keyMessage: 下载即用，全平台；从今天起，有人在意你的眼睛。
字幕文案: ① Look Me · 看看我 ② 刘看山住进你的桌面：提醒眨眼、远眺、起身 ③ macOS · Windows · Linux

Reproduce：lockup 组装 + 卫星元素清除的收尾形态。
Scene 1 (0.0–1.2s): cream 雾白底；看山立绘 spring-pop 落位中央偏左（asymmetric 60/40 留字标位）。
Scene 2 (1.2–2.8s): 字标「Look Me · 看看我」per-word 组装（display ramp）；看山拍手 sprite 一轮彩蛋。
Scene 3 (2.8–4.6s): 副题「刘看山住进你的桌面：提醒眨眼、远眺、起身」lead 滑入；三平台按钮依次生长点亮（`press-release-spring` 平滑）。
Scene 4 (4.6–6.4s): settle hold——完整 lockup 静读。
Scene 5 (6.4–8.0s): 最后一拍：看山眼睑眨一下（呼应 F1），定格至终帧。
