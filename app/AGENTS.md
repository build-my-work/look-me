...
- “有效看屏时长” is a conservative face-visible proxy, not gaze tracking. Accumulate only while local camera sensing is ready, a face is confidently visible, and the coach is neither paused nor in a distance break; show today's duration in compact statistics and the selected day's duration in history using the existing minute-level `observedMs` data.
- Daily history uses two vertically stacked, time-synchronized tracks inside the existing compact card: the upper track is the actual blink count recorded in each observed minute with no partial-minute extrapolation, and the lower track is effective screen-facing seconds for that minute on a fixed 0–60 scale. Fully unobserved minutes remain gaps; short observations remain visible and are contextualized by the screen-facing track.
